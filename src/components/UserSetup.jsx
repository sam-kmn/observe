...
const UserSetup = ({useColumns, useUser}) => {
  ...
  const submit = () => {
    ...
    if (unique.size !== inputCols.length) return warn('Columns have to unique!')
    const data = {}
    ...
    <div className="flex flex-row justify-center items-center h-screen">
      <div className="flex flex-col items-start gap-6"> 
        ...
        <div className="mt-2 flex flex-row gap-5 items-end">
          <button onClick={generateInput} className='flex flex-row gap-1 justify-center items-center text-xl text-indigo-400 hover:text-white hover:bg-indigo-500 p-2 rounded transition duration-400'><AddIcon /> Define new column</button>
          {inputCols.length > 0 && <button onClick={submit} className='flex flex-row gap-1 justify-center items-center text-xl text-green-300 hover:text-white hover:bg-green-500 p-2 rounded transition duration-400'><CheckIcon /> Save columns</button> }
        </div>
